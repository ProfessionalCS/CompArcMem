...
static void probe(const char *name, off_t phys, size_t span) {
    ...
    /* Try a write+readback at offset 0x10 (pio64_out_0 / adder_a) */
    if (sigsetjmp(jmp, 1) == 0) {
        base[4] = 0xCAFEBABE;  /* offset 0x10 */
        __asm__ __volatile__("dsb sy" ::: "memory");
        uint32_t rb = base[4];
        printf("    Write 0xCAFEBABE to [0x10], read back 0x%08X\n", rb);
    } else {
        printf("    Write test at [0x10]: *** BUS ERROR ***\n");
    }
